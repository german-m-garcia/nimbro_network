...
YUVConverter::YUVConverter(int width, int height) : _width(width), _height(height)
{
	U_tmp = new unsigned char[width * height];
	V_tmp = new unsigned char[width * height];
};

void YUVConverter::cRGB_to_YUV420(const unsigned char *rgb, unsigned char *yuv420, int width, int height)
{
	...
	for (int y = 0; y < height; ++y)
	{
		...
		{
			crgb_to_yuv(rgb[3 * (y * width + x) + 0], rgb[3 * (y * width + x) + 1], rgb[3 * (y * width + x) + 2], y_pixel[index], U_tmp[index], V_tmp[index]);
			index++;
		}
	}
...
void YUVConverter::crgb_to_yuv(unsigned char b, unsigned char g, unsigned char r,
